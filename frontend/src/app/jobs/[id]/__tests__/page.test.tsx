...
describe("JobDetail", () => {
  test("renders job details heading", () => {
    render(<JobDetail params={{ id: "test-123" }} />);
    const heading = screen.getByRole("heading", { name: /Job Details/i });
    expect(heading).toBeInTheDocument();
  });

  test("displays the job ID from params", () => {
    const jobId = "abc-123-def";
    render(<JobDetail params={{ id: jobId }} />);
    const idText = screen.getByText(`Job ID: ${jobId}`);
    expect(idText).toBeInTheDocument();
  });

  test("renders status section with loading state", () => {
    render(<JobDetail params={{ id: "test-123" }} />);
  ...
  test("renders results section with placeholder text", () => {
    render(<JobDetail params={{ id: "test-123" }} />);
